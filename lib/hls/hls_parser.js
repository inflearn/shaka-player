...
shaka.hls.HlsParser = class {
  ...
  async convertParsedPlaylistIntoStreamInfo_(playlist, verbatimMediaPlaylistUri,
    ...
    let encrypted = false;
    /** @type {!Array.<shaka.extern.HlsAes128Key>}*/
    const hlsAes128Keys = [];
    ...
    for (const drmTag of drmTags) {
      ...
      if (method == 'AES-128') {
        ...
        hlsAes128Keys.push({method, keyUri, iv, firstMediaSequenceNumber});
      } else if (method != 'NONE') {
        encrypted = true;
    ...
    if (encrypted && !drmInfos.length && !hlsAes128Keys.length) {
      throw new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.MANIFEST,
