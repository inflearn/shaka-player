  ...
  async fetchAndAppend_(mediaState, presentationTime, reference) {
    ...
    try {
      ...
      if (this.config_.lowLatencyMode && isReadableStreamSupported && isMP4) {
        ...
        if (stream.hlsAes128Keys && stream.hlsAes128Keys.length) {
          const seq = iter.currentPosition();
          let sequence = seq;
          const iv = shaka.util.BufferUtils.toUint8(new ArrayBuffer(16));
          for (let i = iv.byteLength - 1; i >= 0; i--) {
            iv[i] = sequence & 0xff;
            sequence >>= 8;
          }
          const hlsParser = new shaka.hls.HlsParser();
          console.log(stream.hlsAes128Keys[seq].keyUri);
          const cryptoKey = await hlsParser.loadKey(
              stream.hlsAes128Keys[seq].keyUri);
          console.log(cryptoKey);
          if (!cryptoKey) {
            return;
          }
          result = await window.crypto.subtle.decrypt(
              {name: 'AES-CBC', iv}, cryptoKey, result);
        }
        this.destroyer_.ensureNotDestroyed();
        if (this.fatalError_) {
          return;
